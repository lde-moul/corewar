...
/*   Updated: 2017/10/17 16:20:51 by lde-moul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	create_process(vm, (src->pc + inst->params[0] % IDX_MOD) % MEM_SIZE, 0, 1);
...
{
	create_process(vm, (src->pc + inst->params[0]) % MEM_SIZE, 0, 1);
	memcpy(vm->processes->r, proc->r, REG_NUMBER);
	vm->processes->carry = proc->carry;
}
