...
/*   Updated: 2017/11/01 18:18:51 by lde-moul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	display_viewed_process(t_vm *vm, WINDOW *info, int line)
{
	...
	if (vm->viewed_process)
	{
		...
		mvwprintw(info, line + 9, 6, "     [%x][%x][%x][%x]", p->r[4], p->r[5],
				p->r[6], p->r[7]);
		mvwprintw(info, line + 10, 6, "     [%x][%x][%x][%x]", p->r[8], p->r[9],
				p->r[10], p->r[11]);
		mvwprintw(info, line + 11, 6, "     [%x][%x][%x][%x]", p->r[12],
...
void	display_info(t_vm *vm)
{
	...
	mvwprintw(info, 3, 3, "Current cycle: %d", vm->cycle);
	mvwprintw(info, 4, 3, "Cycles before check: %-4d", vm->check_cycles);
	mvwprintw(info, 6, 3, "Lives in period: %-10d", vm->num_lives);
	...
	mvwprintw(info, line + 5, 3, "Total lives: %d", vm->tot_lives);
